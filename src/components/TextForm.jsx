...
export default function TextForm(props) {
    ...
    const handleBoldClick = () => {
      ...
      let newText = text.split('').map(char => boldMap[char] || char).join('');
      setText(newText);
      props.showAlert("Text converted to bold Unicode","success");
    };
    const handleLoClick=()=>{
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <motion.div
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  <Button variant="outline" disabled={text.length===0} onClick={handleBoldClick}>Bold</Button>
                </motion.div>
              </TooltipTrigger>
              <TooltipContent>
                <p>Converts the text to bold</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          <TooltipProvider>
